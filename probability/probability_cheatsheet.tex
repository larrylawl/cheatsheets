...
\subsection{Unions, Intersections, and Complements}
    ...
    \begin{description}
        ...
        %      P({\bf A}) &= 1 -  P({\bf A}^c)
        %    \end{align*} 
        \item[Complement of Conditional Probability] 
            \begin{align*}
                P(E|D') &= 1 - P(E'|D')\\
                P(E|D) &= 1 - P(E'|D)
            \end{align*}
...
\textbf{Incremental Bayes' Rule} If $x_1, x_2$ are conditionally independent given h,
...
\begin{description}
\item [Covariance] between two variables is positive when they tend to move in the same direction and negative if they tend to move in opposite directions. 
...
\subsection{Law of Large Nugsxmbers (LLN)}
...
If $X ~ B(n, p)$ and if $n$ is large and $p$ is close to $\frac{1}{2}$, then $X\dot{\,\sim\,}N(np, npq)$. \\
If $X~Pois(l)$ then for large values of l, $X\dot{\,\sim\,}N(l, l)$ approximately.
...
\begin{description}
    \item $ \mu = \frac{p}{q}, \sigma^2 = \frac{1-p}{p^2}$
    \item \( m(t) = p[1 - (1 - p) e^t]^-1, t < -log(1 - p) \)
...
\begin{description}
    \item[Covariance Matrix] Given a vector of random variables $\vec{X} := (X_1, \cdots, X_n)$ with mean $\vec{\mu} = (\mu_1, \cdots, \mu_n)$, its covariance matrix is defined by
    $$ \Sigma_{\vec{X}}:=\left(\Sigma_{i j}\right)_{1 \leq i, j \leq n}=\left(\operatorname{Cov}\left(X_{i}, X_{j}\right)\right)_{1 \leq i, j \leq n}=E\left[(\vec{X}-\vec{\mu})(\vec{X}-\vec{\mu})^{T}\right] = E[XX^T] - \mu\mu^T $$. Source \href{http://cs229.stanford.edu/section/gaussians.pdf}{here}.
    \item[Properties of Covariance Matrix] Suppose that $\Sigma$ is the covariance matrix corresponding to some random vector $X$. Then $\Sigma$ is symmetric positive semidefinite. Source \href{http://cs229.stanford.edu/section/gaussians.pdf}{here}.
    \item[Multivariate Normal] A random vector $\vec{X} = (X_1, \cdots, X_n)$ with mean $\vec{\mu} \in \mathbb{R}^n$ and covariance matrix $\Sigma = (\Sigma_{i,j})_{1 \leq i, j \leq n}$ is called multivariate normal, denoted by $N(\vec{\mu}, \Sigma)$, 
    if $det(\Sigma) \neq 0$ and $\vec{X}$ has pdf
    $$ f_{\vec{X}}(\vec{x})=\frac{1}{\sqrt{(2 \pi)^{n} \operatorname{det}(\Sigma)}} e^{-\frac{1}{2}(\vec{x}-\vec{\mu})^{T} \Sigma^{-1}(\vec{x}-\vec{\mu})} $$

    When n = 2, it is called a \textbf{bivariate normal}.
    \item[Theorem 0] If $\vec{X} = (X_1, \cdots, X_n) \sim (\vec{\mu}, \Sigma)$ with $\Sigma_{ii} = \sigma^2_i$, and $\Sigma_{ij} = 0$ for $i \neq j$, then $X_1, \cdots, X_n$ are \textbf{independent} with $X_i \sim N(\mu_i, \sigma^2_i)$.
    \item[Theorem 1] Given \textbf{i.i.d. standard normals} $\vec{X} = (X_1, \cdots, X_n)$, if $A$ is an $n \times n$ matrix with $det(A) \neq 0$, and $\vec{\mu} \in \mathbb{R}^n$, then $\vec{Y} := A\vec{X} + \vec{\mu} \sim N(\vec{\mu}, \Sigma)$ with $\Sigma = AA^{T}$. 
    Intuitively, Y is a multivariate normal because it is an affine transformation of X, and X is an affine transformation of the standard normal Z. Thus Y can be expressed as an affine transformation of Z. 
    \item[Theorem 2] If $\vec{X} \sim (\vec{\mu}, \Sigma_{\vec{X}})$, and $\vec{Y} = A\vec{X} + \vec{v}$ where $A$ is an $n \times n$ matrix with $det(A) \neq 0$ and $\vec{v} \in \mathbb{R}^n$. Then $\vec{Y} \sim N(A\vec{\mu} + v, \Sigma_{\vec{Y}})$, with 
    \begin{align*}
        \Sigma_{\vec{Y}}&=E\left[(\vec{Y}-E[\vec{Y}])(\vec{Y}-E[\vec{Y}])^{T}\right] \\
        &=E\left[A(\vec{X}-\vec{\mu})\left(A(\vec{X}-\vec{\mu})^{T}\right)\right]=A \Sigma_{\vec{X}} A^{T}
    \end{align*}
    In other words, affine transformations of multivariate normal are also multivariate normal.
    \item[Theorem 3] Let $\vec{X} \sim (\vec{\mu}, \Sigma)$. Then there exist an $n \times n$ matrix $A$ and i.i.d. standard normals $\vec{Z} = (Z_1, \cdots, Z_n)$, such that $\vec{X} = A{Z} + \vec{\mu}$.
    \item[Theorem 4] Let $\vec{X} \sim (\vec{\mu}, \Sigma)$. Let $\vec{Y} = (Y_1, \cdots, Y_m)$ be a subset of $(X_1, \cdots, X_n)$ with $m < n$. Then $\vec{Y} \sim N(\mu_{\vec{Y}}, \Sigma_{\vec{Y}})$, where $\mu_{\vec{Y}} = E[\vec{Y}]$ and $\Sigma_{\vec{Y}}$ is the covariance matrix of $\vec{Y}$.
    In other words, marginal distribution of multivariate normal are normal. 
    % \item[Conditional Distribution] If both $\vec{x}$ and $\vec{y}$ are jointly normal random vectors defined as:
    % $$
    % \left[\begin{array}{l}
    % \mathbf{x} \\
    % \mathbf{y}
    ...
    % A & C \\
    % C^{T} & B
    % \end{array}\right]\right)=\mathcal{N}(\mu, \Sigma), \quad \Sigma^{-1}=\Lambda=\left[\begin{array}{cc}
    % \tilde{A} & \tilde{C} \\
    % \tilde{C}^{T} & \tilde{B}
    % \end{array}\right]
    % $$
    % % TODO: What does the tilde symbols represent?

    % The conditional distribution of $\vec{x}$ given $\vec{y}$ is defined as 
    % $$ p(x|y) = \mathcal{N}(\mu_{x|y}, \Sigma_{x|y}) $$
    % With:
    % $$
    % \begin{array}{l}
    % \Sigma_{x | y}=A-C B^{-1} C^{\top}=\tilde{A}^{-1} \\
    % \mu_{x | y}=\mu_{x}+C B^{-1}\left(\vec{y}-\mu_{y}\right)
    % \end{array}
    % $$
    % With $A-C B^{-1} C^{\top}=\tilde{A}^{-1}$ the Schur complement of B in $\Sigma$. 

    \item[Conditional Distribution] Let $p \times 1$ random vector $Z$ be partitioned into two random vectors $X_1$ and $X_2$ (ie $Z = (X_1 X_2)^T$). 
    ...
    \end{array}\right)
    $$
    The conditional distribution of $X_1$ given known values for $X_2 = x_2$ is defined as:
    $$ p(X_1 | X_2 = x_2) = \mathcal{N}(\mu_{x|y}, \Sigma{x|y}) $$
    With: 
    \begin{align*}
    \text{Mean vector} &= \mu_1 + \Sigma_{12}\Sigma_{22}^{-1}(x_2 - \mu_2) \\
    \text{Covariance matrix} &= \Sigma_{11} - \Sigma_{12}\Sigma_{22}^{-1}\Sigma_{21}
    \end{align*}
\end{description}

%%%%%
% Previous multivariate normal section replaced by ST2131 notes.
%%%%%
...
\textbf{Distribution} & \textbf{PMF/PDF and Support} & \textbf{Expected Value}  & \textbf{Variance} & \textbf{MGF} & \textbf{Tail Probability}\\
\hline 
...
\hline
\shortstack{Geometric \\ \Geom($p$)} & \shortstack{$P(X=k) = q^kp$  \\ $k \in \{$0, 1, 2, \dots $\}$}& $1/p$ & $q/p^2$ & $\frac{p}{1-qe^t}, \, qe^t < 1$ & $(1 - p)^x$ \\
\hline
