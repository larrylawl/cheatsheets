...
\begin{multicols*}{4}
  ...
  \begin{enumerate}
    ...
    \item If $f(x)$ is continuous, $\lim\limits_{x \to c} f(g(x)) = f(\lim\limits_{x \to c} g(x))$.
  \end{enumerate}
  ...
  \( H_{\infty} = \sum^{\infty}_{k=1} \frac{1}{k} \) \\
  \( H_n = \sum^{n}_{k=1} \frac{1}{k} = \gamma+\psi_{0}(n+1) = \Theta(logn) \) \\
  ...
  \subsection*{Quadratic Sequences}
  ...
  E.g.: $ 1, 3, 6, 10 \dots $, where the second difference is 1. \\
  $ T_n = an^2 + bn + c $, where you obtain a, b, and c by subbing $n = 1, 2, or 3$.
  \subsection*{Common Sequences}
  $ \sum i^2 = \frac{(n)(n+1)(2n+1)}{6} $

  \subsection*{CHANGE NAME}
  Let $j_1, j_2, \cdots, j_l$ be any arbitrary $l$ positive integers st $\sum_{t = 1}^l j_t < m$.
  $$ \frac{j_{1}}{m}+\frac{j_{2}}{m-j_{1}}+\frac{j_{3}}{m-j_{1}-j_{2}}+\cdots+\frac{j_{\ell}}{m-j_{1}-j_{2}-\cdots-j_{\ell-1}} \leq \log m $$

  \includegraphics[width=\columnwidth]{change-name-inequality.png}
